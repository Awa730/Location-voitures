...
// ── Données fictives pour démo admin ──
    ...
    <>
      ...
      >
        ...
        <td className="px-4 py-3 hidden lg:table-cell">
          {paiement ? (
            <span className="text-xs font-semibold uppercase tracking-widest px-2.5 py-1 border rounded-sm bg-slate-50 text-slate-600 border-slate-200">
...
export default function AdminDashboard() {
  ...
  const handleValider = (id: number) => {
    handleStatutChange(id, "Confirmée");
  };
      ...
      <main className="max-w-7xl mx-auto px-6 py-8 space-y-8">
        ...
        <div className="flex items-center justify-between">
          ...
          <p className="text-xs text-gray-400">
            Total filtré : <span className="font-bold text-gray-700">
              {filtered.reduce((s, r) => s + (r.type === "location" ? r.prix * calcDuree(r.dateDebut, r.dateFin) : r.prix), 0).toLocaleString("fr-FR")} FCFA
            </span>
          </p>
        </div>
